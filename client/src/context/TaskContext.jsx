...
export const TaskContextProvider = ({ children }) => {
  ...
  const DeleteTask = (id) => {
    //hacemos la funcio la cual recibe un id que nos da al clickear en la papelera

    try {
      // usamos try catch para obtener el error en caso de que alla
  ...
  const updateDone = async (id) => {
    try {
      ...
      await updateDoneRequest(id, taskFound.done === 0 ? true : false);
      loadTasks();
    } catch (error) {
      console.log(error);
    }
